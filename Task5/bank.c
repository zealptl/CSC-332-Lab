...
// ZP - All changed code will be commented with ZP initials
...
int main()
{
	...
	// ZP - keep count of number of attemps to enter CS
	int shmid, *count;
	...
	{
		printf("\nShared memory allocation error occured");
		exit(1);
	}
	*count = 0;

	// ZP - T(P) for three processes
	...
	if (pid == CHILD)
	{
		...
		for (i = 1; i <= N; i++)
		{
			...
			P(mutex);
			{
				// ZP - number of processes in the CS
				t_dad += (*count - temp_count);
				*count += 1;
